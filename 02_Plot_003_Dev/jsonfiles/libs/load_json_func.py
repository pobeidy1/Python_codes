...
def loadDataFile(dataFilePath): #-> List[Dict]:
    """Convenience function to load data from a JSON formatted file"""
...
def extractStudyIdsForTags(tags, data):# -> List[str]:
...
def findStilIdsMatchingStudyIds(studyData, studyIds):# -> Dict[str, Dict]:
...
def saveMeasurementsToCSVFile(filepath, measurementData):
...
def getMeasurements(workspaces, matchingData):#-> List[Dict]:#I changed it from oidStudyDataMap
    data = []
    ...
    for workspace in workspaces:
        ...
        if oid not in matchingData.keys(): #I changed it from oidStudyDataMap
            continue
        ...
        for flow_measurement in flow_measurements.values():
            ...
            point["study_id"] = oid
            point["study_instance_uid"] = matchingData[oid]["study_instance_uid"]#I changed it from oidStudyDataMap
            point["stilid"] = matchingData[oid]["stilid"]#I changed it from oidStudyDataMap
...
def mergeMeasurements(userWiseMeasurements):# -> List[Dict]:
    mergedMeasurements = []
...
def mergeMeasurements1U(userWiseMeasurements):# -> List[Dict]:
    mergedMeasurements1U = []
    users = list(userWiseMeasurements.keys())
    assert len(users) == 1, "Currently this function supports comparing measurements of two users"
    for measurement1 in userWiseMeasurements[users[0]]:
        foundMatching = False
        ...
        for key in measurement1.keys():
            mergedMeasurement[str(key) + "_" + users[0]] = measurement1[key]
        mergedMeasurements1U.append(mergedMeasurement)
    return mergedMeasurements1U